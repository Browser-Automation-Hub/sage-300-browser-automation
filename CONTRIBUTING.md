# Contributing to Sage 300 ERP Browser Automation
...
- **Broken selectors**: Sage 300 ERP updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Sage 300 ERP instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.SAGE_300_URL}/path/to/section`);
...
When you find working selectors for Sage 300 ERP, document them:

```javascript
// Sage 300 ERP selectors (verified YYYY-MM-DD)
...
Sage 300 ERP updates its UI periodically. When selectors break:
